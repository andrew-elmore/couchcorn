...
dev = true
i = 1
puts Time.now if dev


puts "starting seed" if dev
Account.destroy_all
Video.destroy_all
Category.destroy_all
puts "destroyed all" if dev

user1 = Account.create(account_email: "name@website.com", password: 'password')
puts "user Created" if dev
...
puts "video #{i} attached #{Time.now}" if dev
...
puts "video #{i} created #{Time.now}" if dev
i += 1
